...
with open('embeddings.21', 'r') as file:
    ...
    for i, line in enumerate(file.readlines()):
        ...
        # print(v) #check
        char_ix[c] = i + 1  # Indexing the first letter in the embedding from 1. Remember: '#PADDING#': 0. So, the next key in the dictionary cant be zero.
...
def save_file_smy(class_tag):
    ...
    #with open('test.pkl', 'rb') as file:
        #protein_seqence = pickle.load(file)
        #print(protein_seqence)
        #i=0
        #for line in protein_seqence:
            #tp = line.strip("'")
            #print(tp)
            #for j, c in enumerate(line[:M]):
                ##print(j,c) #check
                #ix = char_ix[c]
                #print(ix) #check
                #all_sequences[i][j] = ix
    ...
    np.save('{0}_all_test_labels.npy'.format(class_tag), all_labels)  # all_labels
